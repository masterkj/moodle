    ...
    if ($st = $stmt->fetch()) {
        ...
        if ($Quantity >= $percent) {
            ...
            if ($Quantity < $realcours_opject->price) {
                ...
                echo "The quantity is larger than the price of the course " . $realcours_opject->price;
                return false;
            }
        } else {
...
function first_payment_condition($student_id, $group_id, $Quantity)
{
    $dbclass = new DBClass();
    $connection = $dbclass->getConnection();
    $realcours = new Real_cours($connection);
    ...
    if ($st = $stmt->fetch()) {
        $realcours_opject = json_decode(json_encode($st));
        $percent = $realcours_opject->price * 40 / 100;
        if ($Quantity >= $percent && $Quantity < $realcours_opject->price)
            return true;
    }
    return false;
}
